...
# Sort the DataFrame based on 'store_sales' column
sorted_df = df.sort_values(by='store_sales', ascending=False)

# Select the top 10 stores
top_10_stores = sorted_df.head(10)

# Display the total amount of sales for all stores
total = df['store_sales'].sum()
formatted_total = "${:,.2f}".format(total)
st.header(f'Total Amount of Sales All Stores is {formatted_total}')

# Create a DataFrame with store IDs and their sales
top_10_stores_df = pd.DataFrame({'store_sales': top_10_stores['store_sales'].values}, index=top_10_stores['store_id'])

# Add a toggle button to display the scatter plot for number of customers
show_customer_count = st.checkbox("Show Customer Count")

# If the toggle button is checked, display the scatter plot
...
# Create a bar chart with the top 10 store IDs and their sales
st.subheader("Top 10 Stores based on Sales:")
st.bar_chart(top_10_stores_df)